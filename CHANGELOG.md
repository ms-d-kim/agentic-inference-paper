...
### 2026-06-14 — Triage external Codex brief-review: accuracy + claim-status fixes
Triaged a final Codex review of the master brief and fixed the legitimate findings. **Technical/integrity:** corrected the decode explanation ("a forward pass over K tokens costs ~the same as 1" was wrong → it's weight-read amortization across a batch); reframed the **85–97% figure** as NVIDIA *citing* managed-provider Claude-Code numbers as motivation (not a Dynamo self-hosted benchmark) in all three spots; marked **GAIATrace** release-pending (not yet available); added **BurstGPT 2401.17644** to the ledger (it was ✓ in the brief but missing); fixed **GPU-seconds** "hardware-neutral" → price-neutral/pin-hardware (brief + metric-design); softened ServeGen NSDI'26→claimed and GORGO 2.5×→proxy-vs-prior; corrected the Dynamo schema (agent_hints/cache_control are sibling `nvext` fields), routing (overlap+load), the KV-pool "lineage" overclaim, and flagged the **Agent-Trajectory-Interchange roadmap as UNVERIFIED**. **Framing:** planned C1/C3 no longer read as shipped (added a **[fact]/[plan]/[gate] claim-status legend** to §§7–8 — the review's top ask); "objection answered" → "made testable, still unresolved"; seam = *hypothesized* root cause; RadixAttention = best content-based baseline, not the oracle ceiling; chat-vs-agent now presented as tendencies (agents compact/branch); reframed **Pie** as close architectural prior art; PM value-claims made conditional + a customer-discovery caveat; pilot-matrix caveat (same-stream design). **Completeness:** added a key-serving-terms glossary + a PM/ops-tools line. Bumped Last updated → 21:41 PDT; regenerated PDF. *(Deferred — real work, not edits: rebuild the harness end-to-end and run the micro-pilot.)*
