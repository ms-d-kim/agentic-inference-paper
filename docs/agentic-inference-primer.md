...
**6. The orchestrator/engine seam.** The orchestrator (LangGraph, CrewAI, OpenAI Agents SDK) and the serving engine (vLLM, SGLang) are decoupled black boxes. Neither sees the other's state, so globally optimal cache/scheduling decisions are impossible. "Break the abstraction barrier" is the recurring 2026 systems theme (Sutradhara, Helium).
